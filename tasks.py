...
def sales_commit(product, amount, account, transaction):
    web3_host = os.environ.get('SALES_COMMIT', 'http://52.199.91.197:8000/sales-commit/')
    print('&&&&&')
    print(web3_host)
    data = {
        'product_id': product,
    ...
    if web3_res.status_code in [200, 201, 202] :
        taf_host = os.environ.get('GENERATE_COMMISSION', 'http://52.199.91.197:8000/commission-generate/')
...
def commission_commit(account, commission):
    web3_host = os.environ.get('COMMISSION_COMMIT', 'http://52.199.91.197:8000/commission-commit/')
    data = {
        'account_id': account,
        'number': commission
    }
...
def get_total_commission(account):
    web3_host = os.environ.get('GET_COMMISSION', 'http://52.199.91.197:8000/commission/{}')
    web3_host = web3_host.format(account)
    web3_res = requests.get(url=web3_host)
    if web3_res.status_code in [200, 201, 202]:
        data = web3_res.json()
        taf_host = os.environ.get('POST_COMMISSION', 'http://52.199.91.197:8000/post-commission/')
